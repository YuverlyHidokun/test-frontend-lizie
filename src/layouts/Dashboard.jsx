...
const Dashboard = () => {
  ...
  // Simulación de obtención de datos del usuario (puedes ajustar según tu lógica real)
  useEffect(() => {
    const datosUsuario = JSON.parse(localStorage.getItem('usuario')) || {};
    setUsuario(datosUsuario.nombre || 'Usuario');
    setRol(datosUsuario.rol || 'PASANTE');
  }, []);

  const handleLogout = () => {
    localStorage.removeItem('usuario');
    // Si usas token también:
    ...
    <div className="md:flex md:min-h-screen">
      ...
      <div className="md:w-1/5 bg-gray-800 px-5 py-4">
        ...
        <p className="text-slate-400 text-center my-4 text-sm">
          <span className="bg-green-600 w-3 h-3 inline-block rounded-full"></span> Bienvenido - {usuario}
        </p>
      ...
      <div className="flex-1 flex flex-col justify-between h-screen bg-gray-100">
        ...
        <div className="bg-gray-800 py-2 flex md:justify-end items-center gap-5 justify-center">
          <div className="text-md font-semibold text-slate-100">Usuario - {usuario}</div>
          <div>
            <img
